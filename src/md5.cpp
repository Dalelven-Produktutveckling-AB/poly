...
/* 2017-10-21: Minor modifications for compiling on TI CC1310 
 * 2020-11-11: Modified for C++
 */
...
static void MD5Transform(uint32_t buf[4], const uint8_t inext[64]);

using ::poly::hash::detail::MD5Context;
 ...
 */
static void MD5Final(uint8_t digest[poly::hash::md5::digest_size], struct MD5Context *ctx)
{
    uint32_t count;
    uint8_t *p;
...
void md5::operator()(uint8_t data)
{
    push_back(data);
}

void md5::operator()(etl::span<const uint8_t> buffer)
{
    update(buffer);
}
...
void md5::reset()
{    
    context = detail::MD5Context();
    MD5Init(&context.get<detail::MD5Context>());
}

bool md5::update(etl::span<const uint8_t> buffer)
{
    if (!context.is_type<detail::MD5Context>())
    {
        // Already finalized
        return false;        
    }

    MD5Update(&context.get<detail::MD5Context>(), buffer.data(), (uint32_t)buffer.size());

    return true;
}

void md5::push_back(uint8_t data)
{
    if (!context.is_type<detail::MD5Context>())
    {
        // Already finalized
    ...
    MD5Update(&context.get<detail::MD5Context>(), &data, 1);
}
...
md5::digest md5::finalize()
{
    ...
    {
        digest digest;
        MD5Final(digest.data(), &context.get<detail::MD5Context>());        
        context = digest;
    }

    return context.get<digest>();
}
